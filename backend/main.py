...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://future-image-report.vercel.app"],  # Your frontend
...
FORMULA_DATA = [
    ...
    {"LOB": "PVT CAR", "SEGMENT": "PVT CAR COMP + SAOD",   "PO": "90% of Payin", "REMARKS": "Payin Below 20%"},
    {"LOB": "PVT CAR", "SEGMENT": "PVT CAR COMP + SAOD",   "PO": "90% of Payin", "REMARKS": "Payin Above 20%"},
        {"LOB": "PVT CAR", "SEGMENT": "PVT CAR COMP + SAOD",   "PO": "90% of Payin", "REMARKS": "Payin Above 30%"},
            {"LOB": "PVT CAR", "SEGMENT": "PVT CAR COMP + SAOD",   "PO": "90% of Payin", "REMARKS": "Payin Above 40%"},
                {"LOB": "PVT CAR", "SEGMENT": "PVT CAR COMP + SAOD",   "PO": "90% of Payin", "REMARKS": "Payin Above 50%"},
...
so please start doiing everything accordingly
CRITICAL: Extract EVERY row in the table, even if Weight, State, Zone are repeated.
DO NOT skip or merge rows.
for example if There are 13 rows → must return 26 JSON records (13 Comp + 13 TP).
ALSO , the table is multicolumnar means sometimes , columns are there where columns names based on the Policy Type , so please extract accordingly.


You are extracting insurance policy data from an image. Return a JSON array with these exact keys: segment, policy_type, location, payin, remark.

THIS IS A CV TABLE. FOLLOW THESE RULES STRICTLY:

1. segment = "All GVW & PCV 3W, GCV 3W" for EVERY record
2. location = "FGI STATE: FGI ZONE" → e.g., "BIHAR: NORTH 2"
   - Use EXACTLY the text from FGI STATE and FGI ZONE columns
   - NO extra numbers like "1", "2" unless in image
3. There are EXACTLY 13 rows in the table
   → You MUST return EXACTLY 26 JSON objects (13 Comp + 13 TP)
4. DO NOT skip, merge, or summarize any row
5. OD Out flow → policy_type = "Comp", payin = number (e.g., 19.5)
6. TP Outflow → policy_type = "TP", payin = number
7. remark = "revised wef 1st Aug" for ALL records
8. payin = number only (19.5, not "19.50%")

List of 13 rows to extract (in order):

1. 12K-20K | BIHAR | NORTH 2
2. 20K-40K | BIHAR | NORTH 2
3. Below 2.5 Tons | BIHAR | NORTH 2
4. BOLERO | BIHAR | NORTH 2
5. BOLERO | GUJARAT | CENTRAL
6. Auto | J & K | NORTH 1
7. Below 2.5 Tons | J & K | NORTH 1
8. BOLERO | J & K | NORTH 1
9. 20K-40K | JHARKHAND | NORTH 2
10. BOLERO | JHARKHAND | NORTH 2
11. 12K-20K | KERALA | KERALA
12. 7.5K-12K | KERALA | KERALA
13. Below 2.5 Tons | KERALA | KERALA

For each row, create TWO records: one Comp, one TP.
